...
impl<const N: usize> Bitfield<N> {
    ...
    pub const ORDER: u32 = Self::LEN.ilog2();
    ...
    pub fn set(&self, range: Range<usize>, v: bool) {
        ...
        if range.start != range.end {
            for ei in range.start / Self::ENTRY_BITS..=(range.end - 1) / Self::ENTRY_BITS {
                ...
                if v {
                    unsafe { self.data.get_unchecked(ei) }.fetch_or(byte);
                } else {
                    unsafe { self.data.get_unchecked(ei) }.fetch_and(!byte);
                }
            }
        }
    }
    ...
    pub fn toggle(&self, i: usize, order: u32, expected: bool) -> Result<(), Error> {
        ...
        match order {
            0..=2 => {
                ...
                match unsafe { self.data.get_unchecked(di) }.fetch_update(|e| {
                    if expected {
            ...
            _ => {
                ...
                for i in di..(di + num_entries) {
                    ...
                    if let Err(_e) =
                        unsafe { self.data.get_unchecked(i) }.compare_exchange(expected, !expected)
                    {
                        // log::warn!("Toggle failed {e:x} != {expected:x}");

                        // Undo changes
                        for j in (di..i).rev() {
                            unsafe { self.data.get_unchecked(j) }
                                .compare_exchange(!expected, expected)
                                .map_err(|_| Error::FailedUndoToggle)?;
                        }
    ...
    fn toggle_int<I: Atomic + Default + Not<Output = I>>(
        &self,
        i: usize,
        e: bool,
    ) -> Result<(), Error> {
    ...
    pub fn is_zero(&self, i: usize, order: u32) -> bool {
        let num_bits = 1 << order;
        ...
        if num_bits > Self::ENTRY_BITS {
            ...
            (entry_i..end_i).all(|i| self.data[i].load() == 0)
        } else {
            let entry = self.data[entry_i].load();
    ...
    /// # Warning
    /// Orders above 6 need multiple CAS operations, which might lead to race conditions!
    pub fn set_first_zeros(&self, start_entry: usize, order: u32) -> Result<usize, Error> {
        debug_assert!(start_entry < Self::ENTRIES);

        if order > Self::ENTRY_BITS.ilog2() {
    ...
    fn set_first_zero_entries(&self, order: u32) -> Result<usize, Error> {
        debug_assert!(order > Self::ENTRY_BITS.ilog2());
        debug_assert!(order <= Self::ORDER);

        let num_entries = 1 << (order - Self::ENTRY_BITS.ilog2());
        ...
        for (i, chunk) in self.data.chunks(num_entries).enumerate() {
            ...
            if chunk.iter().all(|e| e.load() == 0) {
                for (j, entry) in chunk.iter().enumerate() {
                    if entry.compare_exchange(0, u64::MAX).is_err() {
                        // Undo previous updates
                        for k in (0..j).rev() {
                            unsafe { chunk.get_unchecked(k) }
                                .compare_exchange(u64::MAX, 0)
                                .map_err(|_| Error::FailedUndoSearch)?;
                        }
                        break;
                    }
                }
...
fn first_zeros_aligned(v: u64, order: u32) -> Option<(u64, usize)> {
    match order {
        0 => {
